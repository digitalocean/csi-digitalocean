...
runner-build:
	...
	@docker pull $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.37 || true
	@docker pull $(CANONICAL_RUNNER_IMAGE):tests-1.37 || true
	@docker pull $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.36 || true
	@docker pull $(CANONICAL_RUNNER_IMAGE):tests-1.36 || true
	@docker pull $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.35 || true
	@docker pull $(CANONICAL_RUNNER_IMAGE):tests-1.35 || true
	...
	@echo "building target tests-1.37"
	@docker build --target tests-1.37 \
		--cache-from $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)builder \
		--cache-from $(CANONICAL_RUNNER_IMAGE):builder \
		--cache-from $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.37 \
		--cache-from $(CANONICAL_RUNNER_IMAGE):tests-1.37 \
		-t $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.37 -f test/e2e/Dockerfile test/e2e

	@echo "building target tests-1.36"
	@docker build --target tests-1.36 \
		--cache-from $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)builder \
		--cache-from $(CANONICAL_RUNNER_IMAGE):builder \
		--cache-from $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.37 \
		--cache-from $(CANONICAL_RUNNER_IMAGE):tests-1.37 \
		--cache-from $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.36 \
		--cache-from $(CANONICAL_RUNNER_IMAGE):tests-1.36 \
		-t $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.36 -f test/e2e/Dockerfile test/e2e

	@echo "building target tests-1.35"
	@docker build --target tests-1.35 \
		--cache-from $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)builder \
		--cache-from $(CANONICAL_RUNNER_IMAGE):builder \
		--cache-from $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.37 \
		--cache-from $(CANONICAL_RUNNER_IMAGE):tests-1.37 \
		...
		-t $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.35 -f test/e2e/Dockerfile test/e2e
...
runner-push: runner-build
	...
	@docker push $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.37
	@docker push $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.36
	@docker push $(RUNNER_IMAGE):$(RUNNER_IMAGE_TAG_PREFIX)tests-1.35
