...
The current version is: **`v0.2.2`**. This means that the project is still
...
For example, to use the latest stable version (`v0.2.2`) you can execute the following command:

```
$ kubectl apply -f https://raw.githubusercontent.com/digitalocean/csi-digitalocean/master/deploy/kubernetes/releases/csi-digitalocean-v0.2.2.yaml
```
...
master with the version `v0.2.2` and then publish a new docker build:
...
This will create a binary with version `v0.2.2` and docker image pushed to
`digitalocean/do-csi-plugin:v0.2.2`
