...
The current version is: **`v1.0.2`**. The project is still
...
For example, to use the latest stable version (`v1.0.2`) you can execute the following command:

```
$ kubectl apply -f https://raw.githubusercontent.com/digitalocean/csi-digitalocean/master/deploy/kubernetes/releases/csi-digitalocean-v1.0.2.yaml
```
...
```
$ make NEW_VERSION=v1.0.2 bump-version
```
...
master with the version `v1.0.2` and then publish a new docker build:
...
This will create a binary with version `v1.0.2` and docker image pushed to
`digitalocean/do-csi-plugin:v1.0.2`
