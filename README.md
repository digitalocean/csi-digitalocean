# DEPRECATION NOTICE: The v0.2.x development branch is deprecated
...
The current version is: **`v0.2.1`**. This means that the project is still
...
For example, to use the latest stable version (`v0.2.1`) you can execute the following command:

```
$ kubectl apply -f https://raw.githubusercontent.com/digitalocean/csi-digitalocean/master/deploy/kubernetes/releases/csi-digitalocean-v0.2.1.yaml
```
...
master with the version `v0.2.1` and then publish a new docker build:
...
This will create a binary with version `v0.2.1` and docker image pushed to
`digitalocean/do-csi-plugin:v0.2.1`
