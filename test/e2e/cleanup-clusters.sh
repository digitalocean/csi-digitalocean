...
while read -r cluster_uuid; do
  if doctl kubernetes cluster get "${cluster_uuid}" --no-header --format Tags | grep -q "${CLUSTER_TAG}"; then
    echo "Deleting cluster ${cluster_uuid} (DRY RUN)"
    echo doctl kubernetes cluster delete -f "${cluster_uuid}"
    echo "Deleting volumes for cluster ${cluster_uuid} (DRY RUN)"
    doctl compute volume list --no-header --format ID,Tags | grep "k8s:${cluster_uuid}" | awk '{print $1}' | xargs -n 1 echo doctl compute volume delete -f
  fi
